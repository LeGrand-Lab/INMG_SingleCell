...
outtable = "TABLE_DOUBLFINDER_oprescuDPI.txt"
source(file="~/INMG_SingleCell/scripts/functions_stock.R", local=T)

observedmaxfeat = 6000  # check scran plots 
# ================================================================================
print("defining full paths raw splitted data")
# ================================================================================
# order not ok <- list.files("data/Oprescu", pattern="DPI", full.names = T)
...
# ================================================================================
print("running Seurat and DoubletFinder using 5% expecte doublets formation")
# ================================================================================
...
for (i in 1:length(ALLFILESTORUN)){
  ...
  seu <- CreateSeuratObject(mat, project=x, min.cells=3, min.features=200) 
  rm(mat)
  # **** NOTE  : trycatch concerns last sample ,
  #    D0_CvDeMich.txt, see end of code
  seu_filtered <- tryCatch({
    seu_filtered <- NormFeatScalePCA(seu, observedmaxfeat, 15)
    return(seu_filtered)
  },error = function(err){
    seu_filtered <- NormFeatScalePCA(seu, observedmaxfeat, 20)
  })
  rm(seu)
  ...
  nExp_poi <- round(0.05*length(seu_filtered@meta.data$orig.ident))  ##NEW: Assuming 5% doublet formation rate
  ...
  colnames(plotdoublet) = c("groupe","nCount_RNA","nFeature_RNA","percent.mt", "RNA_snn_res","cluster",
                            "pANN","DFclass","id","UMAP_1","UMAP_2","TSNE_1","TSNE_2")
  plotdoublet$sample = rep( x,  length(plotdoublet$id) ) #  sample=> needed for prefixes
  metadf = rbind(metadf, plotdoublet)
} 

#  write.table(metadf, paste0(outdir, "tableOprescuSeul_FINDER.txt"), sep="\t", col.names=T)

write.table(metadf, paste0(outdir, outtable), sep="\t", col.names=T)

# OLD version used to produce the very first results for very first comparisons
# ================================================================================
# #
...
#     mat = getMatrixFromCsv(ALLFILESTORUN[[x]]) ; PROJ = y
#   }else {
